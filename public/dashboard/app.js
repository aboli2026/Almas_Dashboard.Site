...
(function () {
  ...
  function cleanupEmptyFolders() {
    let changed = false;
    const allFolders = state.bookmarks.filter(b => b && b.type === 'folder');
    allFolders.forEach(folder => {
      const hasChildren = state.bookmarks.some(b => b && b.parentId === folder.id);
      if (!hasChildren) {
        if (state.currentFolderId === folder.id) {
          state.currentFolderId = folder.parentId || null;
        }
        state.bookmarks = state.bookmarks.filter(b => b.id !== folder.id);
        changed = true;
      }
    });
